...
Model::Model(Shader* shader, Mesh mesh, const char* textureSource, const char* specularMapSource, MaterialType material){
    this->shader = *shader;
    this->mesh = mesh;
...
Model::Model(Shader* shader, Mesh mesh, glm::vec3 color, MaterialType material){
    this->shader = *shader;
    this->mesh = mesh;
    this->materialType = material;
    this->materialConfig = getMaterial(material);
    this->shader.set3f("material.color", color);
    setMaterial();
}

void Model::render(const std::vector<glm::vec3> &positions, float time){
    //get count
...
void Model::render(glm::vec3 position, float time){
    shader.use();
    glBindVertexArray(mesh.getVAO());
    glm::mat4 model = glm::mat4(1.0f);
    model = glm::translate(model, position);
...
MaterialType Model::getMaterialType(){
    return MaterialType();
}
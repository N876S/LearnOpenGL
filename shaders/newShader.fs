...
vec3 getSpotLightFactor(SpotLight light, vec3 lightToFrag, vec3 normal);
...
vec3 getDirLightFactor(DirLight light, vec3 normal, vec3 fragToCamera){
    ...
    //ambient calculation
    vec3 ambient = light.ambient * light.color * vec3(texture(material.diffuse_texture1, texCoord));

    //diffuse calculation
    float diffFactor = max(dot(flippedLightDirection, normal), 0.0f);
    vec3 diffuse = light.diffuse * light.color * vec3(texture(material.diffuse_texture1, texCoord)) * diffFactor;

    //specular calculation
    float specFactor = pow(max(dot(reflectedLight, fragToCamera), 0.0f), 32);
...
vec3 getPointLightFactor(PointLight light, vec3 normal, vec3 fragToCamera){
    ...
    //specular calculation
    float specFactor = pow(max(dot(reflectedLight, fragToCamera), 0.0f), 32);
    vec3 specular = light.specular * light.color * vec3(texture(material.diffuse_texture1, texCoord)) * specFactor;
...
vec3 getSpotLightFactor(SpotLight light, vec3 lightToFrag, vec3 normal){
    ...
    //specular calculation
    float specFactor = pow(max(dot(reflectedLight, -lightToFrag), 0.0f), 32);
    vec3 specular = light.specular * light.color * specFactor;
